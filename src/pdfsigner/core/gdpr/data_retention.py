...
Delegates user anonymization to AnonymizationService.
...
class DataRetentionService:
    ...
    Provides user anonymization (via AnonymizationService), scheduled deletion,
    and automated data purging based on retention policies.
    """
    ...
    def __init__(
        ...
        # Compose anonymization service
        self._anonymization = AnonymizationService(
            user_repository=self.user_repo,
            audit_logger=self.audit_logger,
            anonymize_audit_logs=anonymize_audit_logs,
        )
    ...
    # --- Delegation to AnonymizationService ---
    ...
    def anonymize_user(self, user_id: str, requested_by: str) -> AnonymizationResult:
        ...
        Delegates to AnonymizationService.
        ...
        """
        return self._anonymization.anonymize_user(user_id, requested_by)

    def _is_user_anonymized(self, user_id: str) -> bool:
        """Check if user is already anonymized. Delegates to AnonymizationService."""
        return self._anonymization._is_user_anonymized(user_id)

    def _mark_user_anonymized(self, user_id: str) -> None:
        """Mark user as anonymized. Delegates to AnonymizationService.

        Raises:
            sqlite3.Error: If the database update fails.
        """
        self._anonymization._mark_user_anonymized(user_id)

    # --- Scheduling and Purging ---
