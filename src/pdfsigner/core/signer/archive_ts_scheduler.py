...
Orchestrates periodic archive timestamp checks using SQLite storage
and ArchiveTimestampManager for PAdES-LTA compliance.
"""
...
from pdfsigner.core.signer.archive_ts_storage import ArchiveTSStorage

# Re-export types for backward compatibility
from pdfsigner.core.signer.archive_ts_types import PendingPDF, RegisteredPDF
...
class ArchiveTSScheduler:
    ...
    Uses SQLite (via ArchiveTSStorage) to track registered PDFs and their
    timestamp status. Integrates with ArchiveTimestampManager for checking
    ...
    def __init__(self, db_path: Path | None = None):
        ...
        self._storage = ArchiveTSStorage(self.db_path)
    ...
    def register_pdf(
        ...
        self._storage.insert_pdf(pdf_path, registered_at, check_interval_days, hash_sha256)
        logger.info(
    ...
    def unregister_pdf(self, pdf_path: Path) -> bool:
        ...
        """
        pdf_path = pdf_path.resolve()
        result = self._storage.delete_pdf(pdf_path)
    ...
    def get_pending_pdfs(
        ...
        for reg_pdf in registered:
            ...
            if not pdf_path.exists():
                ...
                self._storage.update_last_checked(pdf_path, now)
                continue
            ...
            if current_hash and reg_pdf.hash_sha256 and current_hash != reg_pdf.hash_sha256:
                ...
                self._storage.update_hash(pdf_path, current_hash)
    ...
    def check_and_update(
        ...
        try:
            ...
            # Update database
            self._storage.update_last_timestamp(pdf_path, now)

            # Update hash
            new_hash = self._compute_hash(pdf_path)
            if new_hash:
                self._storage.update_hash(pdf_path, new_hash)
    ...
    def _check_deprecation_reason(
        ...
        Delegates to deprecation_checker module for the actual inspection.
        ...
        Returns:
            ...
            otherwise ``None``.
        """
        from pdfsigner.core.signer.deprecation_checker import check_deprecation_reason

        return check_deprecation_reason(manager, pdf_path)
