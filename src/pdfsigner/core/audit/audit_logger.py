...
class AuditLogger:
    ...
    - Query interface with date and type filters (via AuditQueryService)
    - CSV export capability (via AuditQueryService)
    """
    ...
    def __init__(
        ...
        # Initialize query service for read operations
        self._query_service = AuditQueryService(self.log_dir, self.enabled)
    ...
    def _write_event(self, event: AuditEvent) -> None:
        ...
        In HIPAA context (SS164.312(b)), silencing write errors is a compliance
    ...
    # -- Query delegation methods (backward-compatible) --
    ...
    def get_events(
        ...
        """
        Query events with filters. Delegates to AuditQueryService.
        ...
        """
        return self._query_service.get_events(start_date, end_date, event_types)
    ...
    def get_events_filtered(
        ...
        Delegates to AuditQueryService.
        ...
        """
        return self._query_service.get_events_filtered(
            start_date, end_date, event_types, user_id, session_id, phi_accessed, status, limit
        )

    def export_csv(self, events: list[AuditEvent]) -> str:
        """
        Export events to CSV format. Delegates to AuditQueryService.
        ...
        """
        return self._query_service.export_csv(events)
